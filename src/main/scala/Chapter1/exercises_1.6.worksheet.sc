...
// 1.6.2.3 Given a function `f: Int => Boolean`, an integer `n` is called a 
// "3-f" if there are only three different integers `1 < i < j < k < n` such
// that f(i), f(j), and f(k) are all true. Defiine a function that takes f as
// an argument and returns a sequence of all "3-f" integers among 
// `n in (1 to 1000)`. What is the type of that function? Implement Exercise 
// 1.6.2.2. using that function
{
  import exercise2.countFactors 
  
  def countFactors2(n: Int): Boolean = countFactors(n) == 3

  def result(f: Int => Boolean): List[Int] =
    (1 to 1000).filter(n => f(n)).toList
  
  result(countFactors2)
}

// 1.6.2.4 Define a function `at100` of type List[List[Int]] => List[List[Int]]
// that selects only those inner lists whose largest value is at least 100. Test
{
  def at100(outer: List[List[Int]]): List[List[Int]] = 
    outer.filter(inner => inner.filter(k => k >=100).length >= 1)

  at100(List(List(0, 1, 100), List(60, 80), List(1000)))
}

// 1.6.2.5 Define a function of type `List[Double] => List[Double]` that performs
// a "normalization" of a list: it finds the element having the largest absolute
// value and, if that value is zero, returns the original list; if that value is 
// nonzero, divides all elements by that value and returns a new list. Test.
{
  def normalize(list: List[Double]): List[Double] =
    val temp: Double = list.map(k => math.abs(k)).max
    if (temp == 0) then list else list.map(k => k / temp) 

  normalize(List(1.0, -4.0, 2.0)) 
}
